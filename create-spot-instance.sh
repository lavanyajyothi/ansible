...
CREATE() {
  ...
  if [ $COUNT -eq 0 ]; then
    aws ec2 run-instances --launch-template LaunchTemplateId=lt-060bf846c161cbde6,Version=1 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
  else
  ...
  aws route53 change-resource-record-sets --hosted-zone-id Z07250483J27MWFJ6RU7C --change-batch file:///tmp/record.json | jq  &>/dev/null
}
